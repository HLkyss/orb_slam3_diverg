...
Sophus::SE3f System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    if(settings_ && settings_->needToRectify()){    //pinhole相机模型会使用
...
//        cv::imwrite("/media/hl/Stuff/ubuntu_share_2/Dataset/ue_stereo_test/test/imRightToFeed.png", imRightToFeed);
    ...
    else{   // 鱼眼图像不修正，见github的readme：For FishEye camera model, rectification is not required since system works with original images
